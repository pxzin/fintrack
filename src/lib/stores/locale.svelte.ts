...
let currentLocale = $state<Locales>(defaultLocale)
...
export function formatCurrency(amount: number, locale?: Locales): string {
	...
	const options = formatOptions[targetLocale] || formatOptions['pt-br']
...
export function formatDate(date: Date, locale?: Locales): string {
	...
	const browserLocale = localeMap[targetLocale] || 'pt-BR'
