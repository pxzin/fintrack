<script lang="ts">
	...
	function initDevStores() {
		...
		for (const [key, entry] of Object.entries(devStores as Record<string, any>)) {
			try {
	...
	function renderLimited(obj: any, depth: number, seen = new WeakSet(), current = 0): string {
		...
		const entries = Array.isArray(obj)
			...
			: Object.fromEntries(
					Object.entries(obj).map(([k, v]) => [k, renderLimited(v, depth, seen, current + 1)])
				);
		try {
