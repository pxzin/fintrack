...
export async function runMigrations() {
	try {
		...
		for (let i = 0; i < tableStatements.length; i++) {
			...
			try {
				console.log(`   Table ${i + 1}/${tableStatements.length}: ${statement.substring(0, 50)}...`)
		...
		if (otherStatements.length > 0) {
			...
			for (let i = 0; i < otherStatements.length; i++) {
				...
				try {
					console.log(`   Other ${i + 1}/${otherStatements.length}: ${statement.substring(0, 50)}...`)
					await executeQuery(statement)
				} catch (error) {
		...
		for (let i = 0; i < indexStatements.length; i++) {
			...
			try {
				console.log(`   Index ${i + 1}/${indexStatements.length}: ${statement.substring(0, 50)}...`)
				await executeQuery(statement)
			} catch (error) {
...
export async function checkDatabaseHealth() {
	try {
		...
		const requiredTables = ['users', 'sessions', 'accounts', 'categories', 'transactions', 'recurrence_adjustments']
